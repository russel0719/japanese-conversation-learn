...
export default function WrongPage() {
  ...
  if (wrongs.length === 0) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#111113] flex flex-col items-center justify-center gap-4 px-6">
        ...
        <div className="text-xl font-semibold text-gray-800 dark:text-gray-100">오답이 없어요!</div>
        <div className="text-sm text-gray-500 dark:text-gray-400 text-center">퀴즈에서 틀린 표현이 여기에 저장됩니다.</div>
        <button
          onClick={() => router.back()}
  ...
  if (!started) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-[#111113] flex flex-col">
        <div className="bg-red-600 text-white px-4 pt-10 pb-4">
          <div className="max-w-2xl mx-auto">
            <div className="text-xl font-semibold">❌ 오답 노트</div>
        ...
        <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-5">
          ...
          <div className="space-y-2 mb-6">
            {wrongs.slice(0, 10).map((w) => (
              <div key={w.phrase.japanese} className="bg-white dark:bg-[#0b0b0c] rounded-xl p-3 border border-red-100 dark:border-red-950 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-red-100 dark:bg-red-950 text-red-500 flex items-center justify-center text-xs font-bold flex-shrink-0">
                  {w.count}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{w.phrase.japanese}</div>
                  <div className="text-xs text-gray-400 dark:text-gray-600">{w.phrase.korean}</div>
                </div>
                <button
                  onClick={() => removeWrongAnswer(w.phrase.japanese)}
                  className="w-11 h-11 flex items-center justify-center text-xs text-gray-300 hover:text-red-400 active:scale-95 transition-all"
                  title="삭제"
                >
                  ✕
                </button>
          ...
          <button
            ...
            className="w-full py-3 bg-red-600 text-white rounded-full font-semibold active:scale-95 transition-transform min-h-[44px]"
          >
            오답 퀴즈 시작
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#111113] flex flex-col">
      <div className="bg-red-600 text-white px-4 pt-10 pb-4">
        <div className="max-w-2xl mx-auto">
          <div className="text-xl font-semibold">❌ 오답 퀴즈</div>
        </div>
      </div>
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-5">
        <QuizEngine
